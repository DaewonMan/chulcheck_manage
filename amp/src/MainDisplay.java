...
public class MainDisplay implements ActionListener{
	...
	JButton byPerson, byLecture, byPolicy,  Application, Cancel, extProgram;
	JButton cPerson, cLecture, attendPerson, absencePerson, latePerson, leavePerson;
	...
	JComboBox box;
	static int [][] appPerson = new int[10][10]; // 수강 신청한 사람과 과목을 할당할 변수
	...
	public MainDisplay(){
		...
		byPerson.setBounds(120, 90, 150, 30);
		//byPerson.setFont(new Font("돋움", Font.BOLD, 15)); // 글씨체 수정
		byPerson.addActionListener(this);
		iFrame.add(byPerson);

		cPerson=new JButton("등록 취소"); // 등록 취소 버튼
		cPerson.setBounds(290, 90, 150, 30);
		//cPerson.setFont(new Font("돋움", Font.BOLD, 15)); // 글씨체 수정
		cPerson.addActionListener(this);
		iFrame.add(cPerson);
		
		byLecture=new JButton("과목 등록"); // 과목 등록 버튼
		byLecture.setBounds(120, 140, 150, 30);
		byLecture.addActionListener(this);
		iFrame.add(byLecture);
		
		cLecture=new JButton("과목 폐강"); // 폐강 버튼
		cLecture.setBounds(290, 140, 150, 30);
		cLecture.addActionListener(this);
		iFrame.add(cLecture);
		...
		Application.setBounds(120, 240, 150, 30);
		Application.addActionListener(this);
		iFrame.add(Application);
		
		Cancel=new JButton("수강 취소"); // 수강 취소
		Cancel.setBounds(290, 240, 150, 30);
	...
	private void load() {
		try {
			...
			for(int i = 0; i < 10;i++){ // applicationList.txt를 한 줄씩 읽어들인다.
				for(int j = 0;j < 10;j++){
	...
	public void actionPerformed(ActionEvent iEvent) { // 버튼 클릭했을 때
		if(iEvent.getSource()==byPerson) { // 수강버튼 클릭 시
		...
		else if(iEvent.getSource()==Cancel){
			int lecture = lectureTable.getSelectedRow();
			if(model.getValueAt(lecture, 2)==null){
				JOptionPane.showMessageDialog(temp, "이 책을 빌린사람이 아무도 없습니다");
			}
			else{
				String personname = (String) model.getValueAt(lecture, 2);
				cancel(personname, (String) model.getValueAt(lecture, 0));
				refresh();
			}			
		}
			...
			try {
				...
				if(iEvent.getSource()==attendPerson) {
					...
					libPerson[attNum].setGrade(1); // 출석 점수 증가
				}
		...
		else // 프로그램 종료
		{
			try {
				...
				for(int i = 0;i < 10;i++) { // 수강신청 목록 파일에 쓰기
					for(int j = 0;j < 10;j++) {
	...
	private void cancel(String personname, String lecturename) {
		for (int i = 0; i < countPerson; i++) {
			if(libPerson[i].getName().equals(personname)){
				//libPerson[i].setNumofLecture(libPerson[i].getNumofLecture()-1);
				break;
			}
		}
		for (int i = 0; i < countLecture; i++) {
			//if(libLecture[i].getLecture().equals(lecturename)){
				//libLecture[i].setPersonname(null);
				//return;
			//}
		}
	}
